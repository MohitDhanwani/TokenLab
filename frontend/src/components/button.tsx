...
export const Button = ({ varient, disabled, name, classname, onClick, type }: ButtonProps) => {
  const varients = {
    primary:
  ...
  return (
    <button disabled={disabled} className={`${varients[varient]} ${classname}`} onClick={onClick} type={type}>
      {name}
    </button>
  );
};
